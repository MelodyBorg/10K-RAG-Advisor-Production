...
# OpenAI (GPT) cloud tiers. Two distinct tiers so they can be compared:
#   gpt4o -> gpt-4o        fast, cheaper general-purpose cloud model
#   gpt5  -> gpt-5.4       premium reasoning model for hard analytical queries
# Override the tags via env vars if you want to swap the exact snapshots.
GPT4O_MODEL = os.getenv("GPT4O_MODEL", "gpt-4o")
GPT5_MODEL = os.getenv("GPT5_MODEL", "gpt-5.4")
...
def get_embeddings(mode: str):
    ...
    """
    if mode in ("gemini", "qwen", "gpt4o", "gpt5"):
        # The Gemini, Qwen, and both OpenAI (GPT) tiers all query the SAME
        # faiss_index_gemini vector store, so they must share the Gemini
        # embedding space. The LLM used for generation is independent of the
        # embedding model, so GPT can generate answers over Gemini-embedded
        # chunks with no index rebuild (same trick the Qwen tier uses).
...
def get_llm(mode: str):
    ...
    elif mode in ("gpt4o", "gpt5"):
        # Cloud GPT tiers. Uses langchain-openai's ChatOpenAI, which reads
        # OPENAI_API_KEY from the environment automatically. Both models share
        # this branch; only the model id differs.
        from langchain_openai import ChatOpenAI
        if not os.environ.get("OPENAI_API_KEY"):
            raise ValueError("OPENAI_API_KEY environment variable is not set.")
        model_id = GPT4O_MODEL if mode == "gpt4o" else GPT5_MODEL
...
def main():
    ...
    parser.add_argument("--mode", type=str, default="gemini", choices=["gemini", "gpt4o", "gpt5", "qwen", "ollama"])
